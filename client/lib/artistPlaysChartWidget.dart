...
class ArtistPlaysChartWidget extends StatefulWidget {
	final Artist artist;

	ArtistPlaysChartWidget(this.artist);
	createState() => _ArtistPlaysChartWidgetState();
}

class _ArtistPlaysChartWidgetState extends State<ArtistPlaysChartWidget> {
	...
	Future<LinkedHashMap<String, int>> refresh() async {
		int total = 0;
		final earliestPlay = await server.getEarliestArtistPlay(widget.artist);
		final latestPlay = await server.getLatestArtistPlay(widget.artist);
		...
		return await server.getPlayPeriodCounts(artist: widget.artist, startDate: startDate, endDate: endDate, period: period, limit: barCount);  //TODO Remove offset from this function as dates are now used instead
	}
