...
class ArtistPlaysListWidget extends StatefulWidget {
	final Artist artist;

	ArtistPlaysListWidget(this.artist);
	createState() => _ArtistPlaysListWidgetSate();
}

class _ArtistPlaysListWidgetSate extends State<ArtistPlaysListWidget> {
	...
	build(context){
		...
		Future<List<Play>> refresh() async => await server.getArtistPlays(artist, limit: itemCount);
