...
class ArtistCountsChartWidget extends StatefulWidget {
	
	createState() => _ArtistCountsChartWidgetState();
}

class _ArtistCountsChartWidgetState extends State<ArtistCountsChartWidget> {
	...
	Future<LinkedHashMap<String, int>> refresh() async {
		...
		return await server.getArtistPeriodCounts(startDate: startDate, endDate: endDate, period: period, limit: barCount);
	}
	...
	build(context){
		...
		titleBar(){
			return Row(
				children: [
					Expanded(
						child: IgnorePointer(child: Text("Total Artists", style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold))),
					),
					dropDown()
				],
			);
