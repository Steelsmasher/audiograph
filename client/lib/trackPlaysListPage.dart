...
class TrackPlaysListPage extends StatefulWidget {
	final Track track;

	TrackPlaysListPage(this.track);

	createState() => _TrackPlaysListPageState();
}

const _playsPerPage = 50;

class _TrackPlaysListPageState extends State<TrackPlaysListPage> {
	...
	Future<List<Play>> refresh() async {
		...
		final trackCount = await server.getTrackCount(track);
		totalPages = (trackCount/_playsPerPage).ceil();
		final offset = (currentPage-1)*_playsPerPage;
		return await server.getTrackPlays(track, limit: _playsPerPage, offset: offset);
	}

	ListView getTrackPlayList(List<Play> plays){
